...
// -- Get all non-Java library methods from top of call stack
MethodDeque Thread::top_nonjavalib_methods()
{
    MethodDeque result;
    if (this->m_kind == ExecMode::Full) {
        // TODO: NOT IMPLEMENTED YET FOR FULL MODE
        ...
        unsigned int count = 0;
        while (count < this->m_methods.size()) {
            Method *mptr = this->TopMethod(count);
            result.push_front(mptr);
            count++;
        }
                                        ...
                                        MethodDeque &methdeque )
{
    ...
    while (count < methdeque.size()) {
        if (count < 2) {
            string next_name;
            Method *next_method = NULL;
            next_method = methdeque[methdeque.size() - count - 1];
            count += 1;
...
// OLD VERSION 1:
//    - This was the version used for Onwards 2017. It looks like
//      though that the handling of javalib methods vs nonjavalib methods 
//      could be problematic (ie buggy). - RLV Aug 5, 2019
// Update the Object pointer to simple Death context pair map
void ExecState::UpdateObj2DeathContext_version_1( Object *obj,
                                                  MethodDeque &methdeque )
{
    unsigned int count = 0;
    bool nonjava_flag = false;
    ...
    while (count < 2) {
        string next_name;
        Method *next_method = NULL;
        if (methdeque.size() >= (count + 1)) {
            next_method = methdeque[count];
        } else {
            break;
        }
        count += 1;
